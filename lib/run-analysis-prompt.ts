...
export function buildRunAnalysisPrompt(run: RunRecord): string {
  const lines: string[] = [
    ...
    '3. Give a practical "how to reach" guide for the next 1 to 3 stops: turn-by-turn style checkpoints, landmark-style cues, and when to switch to external navigation.',
    '4. List 3–5 operational tips for this specific sequence (timing, breaks, fuel, difficult legs).',
    '5. Flag any risks (long legs, tight ETAs, many stops in a row) and one alternative heuristic if a stop fails.',
    '6. Keep the tone actionable; avoid inventing addresses or stops not listed.',
    '',
    '## Run data',
    `- **Run ID:** ${run.runId}`,
